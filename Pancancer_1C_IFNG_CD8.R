...
VlnPlot(pbmc_CD8,features = "IFNG",sort = TRUE,pt.size = 0)
...
for (i in 1:length(patient)) {
  ...
  assign("f",select(e,one_of("IFNG")))
  assign(paste("scaldata_",i,"_TIL",sep = ""),data.frame(mean(c$JMJD1C),mean(f$IFNG)))
...
colnames(scaldata)<-c("JMJD1C","IFNG_CD8")
library(ggplot2)
library(ggpubr)
ggplot(data = scaldata,aes(x=JMJD1C,y=IFNG_CD8)) + geom_point(color="blue",size=3.4) + stat_smooth(method = "lm",se=TRUE,size=1.5,color="red")+stat_cor(data = scaldata,method = "spearman") +  theme_set(theme_bw()) +  theme_set(theme_bw()) 
