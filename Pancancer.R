...
J<-metadata_new
rownames(J)<-colnames(F)

pbmc <- CreateSeuratObject(counts = F, project = "pbmc3k", min.cells = 3, min.features = 200,meta.data = J )
...
Treg<-subset(pbmc,idents = c("19","4","14","0","11","18"))
...
for (i in 1:length(patient)) {
  ...
  colnames(e)<-"STAT3"
  assign(paste("scaldata_",i,"_TIL",sep = ""),data.frame(sum(c$JMJD1C),sum(e$STAT3)))
...
colnames(scaldata)<-c("JMJD1C","STAT3_signaling")
library(ggplot2)
library(ggpubr)
ggplot(data = scaldata,aes(x=JMJD1C,y=STAT3_signaling)) + geom_point(color="blue",size=3.4) + stat_smooth(method = "lm",se=TRUE,size=1.5,color="red")+stat_cor(data = scaldata,method = "spearman") +  theme_set(theme_bw()) +  theme_set(theme_bw()) 
