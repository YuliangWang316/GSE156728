...
a<-read.table("D:/GSE156728/geneset.txt",sep = "\t",header = TRUE)
b<-read.table("D:/GSE156728/geneset (1).txt",sep = "\t",header = TRUE)
...
for (i in 1:length(patient)) {
  ...
  assign("c",select(b,one_of("IFNG")))
  assign("d",slot(get(paste("Treg_",i,"_TIL",sep = "")),"meta.data"))
  STAT3<-select(d,starts_with("STAT3"))
  d$STAT3<-apply(STAT3,MARGIN = 1,mean)
  PI3K<-select(d,starts_with("PI3K"))
  d$PI3K<-apply(PI3K,MARGIN = 1,mean)
  assign("e",as.data.frame(d$STAT3))
  colnames(e)<-"STAT3"
  assign(paste("scaldata_",i,"_TIL",sep = ""),data.frame(sum(c$IFNG),sum(e$STAT3)))
...
colnames(scaldata)<-c("IFNG","STAT3_signaling")
library(ggplot2)
library(ggpubr)
ggplot(data = scaldata,aes(x=IFNG,y=STAT3_signaling)) + geom_point(color="blue",size=3.4) + stat_smooth(method = "lm",se=TRUE,size=1.5,color="red")+stat_cor(data = scaldata,method = "spearman") +  theme_set(theme_bw()) +  theme_set(theme_bw()) 
write.table(scaldata,file = "D:/GSE156728/1C_TregSTAT3signaling.txt",sep = "\t")
