...
for (i in 1:length(patient)) {
  ...
  colnames(e)<-"PI3K"
  assign(paste("scaldata_",i,"_TIL",sep = ""),data.frame(sum(c$IFNG),sum(e$PI3K)))
...
colnames(scaldata)<-c("IFNG","PI3K_signaling")
library(ggplot2)
library(ggpubr)
ggplot(data = scaldata,aes(x=IFNG,y=PI3K_signaling)) + geom_point(color="blue",size=3.4) + stat_smooth(method = "lm",se=TRUE,size=1.5,color="red")+stat_cor(data = scaldata,method = "spearman") +  theme_set(theme_bw()) +  theme_set(theme_bw()) 
write.table(scaldata,file = "d:/GSE156728/1C_TregPI3Ksignaling.txt",sep = "\t")
