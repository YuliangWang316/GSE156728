...
pbmc_T<-subset(pbmc,idents = "T")
Idents(pbmc_T)<-pbmc_T@meta.data$patient
for (i in 1:length(patient)) {
  ...
  assign(paste("pbmc_",i,"_TIL",sep = ""),subset(pbmc_T,idents = patient[i]))
  assign("d",length(colnames(get(paste("Treg_",i,"_TIL",sep = ""))))/length(colnames(get(paste("pbmc_",i,"_TIL",sep = "")))))
...
colnames(scaldata)<-c("JMJD1C","Treg_CD4")
library(ggplot2)
library(ggpubr)
ggplot(data = scaldata,aes(x=JMJD1C,y=Treg_CD4)) + geom_point(color="blue",size=3.4) + stat_smooth(method = "lm",se=TRUE,size=1.5,color="red")+stat_cor(data = scaldata,method = "spearman") +  theme_set(theme_bw()) +  theme_set(theme_bw()) 
